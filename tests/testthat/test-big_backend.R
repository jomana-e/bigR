...
test_that("big_backend switches between available backends", {
  # Test switching to each available backend
  backends <- c("data.table", "arrow", "duckdb")
  
  for (backend in backends) {
    # Switch backend
    old_backend <- big_backend(backend)
    
    # Verify backend was switched
    expect_equal(big_backend(), backend)
    
    # Switch back
    big_backend(old_backend)
  }
})
...
test_that("big_backend maintains state between operations", {
  ...
  expect_s3_class(result_arrow, "data.frame")
  
  # Results should be equal regardless of backend
  expect_equal(result_dt, result_arrow)
})

test_that("big_backend handles backend-specific features", {
  df <- generate_test_data(n_rows = 1000)
  
  # Test DuckDB-specific SQL features
  big_backend("duckdb")
...
test_that("big_backend preserves data types across backends", {
  ...
  for (backend in backends) {
    big_backend(backend)
    results[[backend]] <- df %>%
      big_select(num_1, date, factor, logical) %>%
      collect()
...
test_that("big_backend handles large datasets efficiently", {
  ...
  for (backend in backends) {
    ...
    expect_true(all(c("cat_1", "mean_num", "count") %in% names(result)))
  }
  
  # All backends should complete in reasonable time
  expect_true(all(times < 60))
})
...
test_that("big_backend handles concurrent operations", {
  ...
  big_backend("data.table")
  op1 <- df1 %>% big_filter(num_1 > 0)
  
  big_backend("arrow")
  op2 <- df2 %>% big_filter(num_1 < 0)
...
test_that("big_backend fails gracefully with invalid inputs", {
  ...
  expect_error(big_backend("invalid_backend"))
  
  # NULL backend
  expect_error(big_backend(NULL))
  
  # Empty string backend
  expect_error(big_backend(""))
  
  # Numeric backend
  expect_error(big_backend(1))
})

test_that("big_backend handles temporary backend switches", {
  original_backend <- big_backend()
  
  # Temporary switch to arrow
  withr::with_options(
    list(bigr.backend = "arrow"),
    {
      expect_equal(big_backend(), "arrow")
      df <- generate_test_data(n_rows = 100)
      result <- df %>% big_filter(num_1 > 0) %>% collect()
      expect_true(is.data.frame(result))
    }
  )
  
  # Should revert to original backend
  expect_equal(big_backend(), original_backend)
})
